...
1. **要約**: 質問に対する結論を簡潔に
2. **詳細分析**: 数値やデータに基づく詳細説明（例: 「Tシャツカテゴリの売上は前月比15%増です」）
3. **背景・要因**: 数値変動や定性情報の傾向の理由や背景（例: 「これは夏物新作セールが主要因です」）
4. **比較・評価**: 他のカテゴリや地域との比較
5. **データソース**: 参照したデータソース名と期間を明記（例: 「期間: 2025年6月1日〜6月30日、ソース: SALES, SUPPORT_CASES」）
...
- `SALES`、`PRODUCTS`、`MARKETING_CAMPAIGN_METRICS`、`SOCIAL_MEDIA`、`SUPPORT_CASES` といったテーブルを活用し、多角的にデータを組み合わせた分析を実施
- `CATEGORY`、`REGION` などのカラムを活用したセグメント別分析
- `TRANSCRIPT` の自然言語処理（NLP）により、顧客の感情（センチメント）や頻出キーワードを抽出
- `DATE` を活用した時系列分析
